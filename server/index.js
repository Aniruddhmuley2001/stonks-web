...
mongoose.connect('mongodb+srv://dcei-stonks-2021:dcei-stonks-123@stonks-cluster.5u5xc.mongodb.net/myFirstDatabase?retryWrites=true&w=majority',{
  ...
  if(!stock){
    ...
    if(!user){
      ...
      if(user.credits < stock.price * quantity){
        ...
        let holding = await Holding.findOne({stockId:stock._id,userId:user._id}).exec()
  ...
  if(!user){
    ...
    if(!holding){
      ...
      if(holding.quantity < quantity){
        res.send(`You hold ${holding.quantity} share and not ${quantity} shares`)
      }else{
